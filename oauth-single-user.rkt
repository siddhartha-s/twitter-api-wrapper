...
#|
Developer note: modified `get-request` and `post-request` methods to conver the
JSON outputted from input-port into a hash. 

The original file below provides an implementation of OAauth1 in Racket. It abstracts the
GET and POST requests and responses required during the handshake process for
OAuth1 authentication and is vital in terms of accessing Web APIs which still
use OAuth1.0 authentication.

Source:  https://github.com/StephenCharles/Racket-OAuth1.0a
Author: GitHub user Stephen Charles
|#
